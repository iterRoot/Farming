...
public class PurchaseBlanketAgreementPdfDocument : IDocument
{
    private readonly PurchaseBlanketAgreement _agreement;
    private readonly CompanyEntity?           _company;

    public PurchaseBlanketAgreementPdfDocument(PurchaseBlanketAgreement agreement, CompanyEntity? company)
    {
    ...
    private void ComposeHeader(IContainer container)
    {
        container.Row(row =>
        {
            ...
            {
                col.Item().AlignRight().Text("PURCHASE BLANKET AGREEMENT")
                    .FontSize(15).Bold().FontColor(Colors.Blue.Darken2);

                col.Item().AlignRight().Text($"Doc No: {_agreement.DocNum}")
                    .FontSize(10).Bold();

                col.Item().AlignRight().Text(_agreement.Status == "C" ? "Closed" : "Open")
    ...
    private void ComposeVendorInfo(IContainer container)
    {
        container.Row(row =>
        {
            row.RelativeItem().Column(col =>
            {
                ...
                col.Item().Text(_agreement.Vendor.CardName).Bold();
                col.Item().Text($"Vendor Code: {_agreement.Vendor.Code}").FontSize(9);
            });
            ...
            row.RelativeItem().Column(col =>
            {
                col.Item().Row(r =>
                {
                    ...
                    r.RelativeItem().AlignRight().Text(_agreement.PostingDate?.ToString("dd/MM/yyyy") ?? "-");
                });
                col.Item().Row(r =>
                {
                    r.RelativeItem().Text("Delivery Date").FontSize(9).FontColor(Colors.Grey.Darken1);
                    r.RelativeItem().AlignRight().Text(_agreement.DeliveryDate?.ToString("dd/MM/yyyy") ?? "-");
                });
            });
        });
    }
    ...
    private void ComposeLineItemsTable(IContainer container)
    {
        container.Table(table =>
        {
            ...
            var i = 0;
            foreach (var line in _agreement.Items)
            {
                i++;
                var idx = i;
    ...
    private void ComposeFooterBlock(IContainer container)
    {
        container.Row(row =>
        {
            row.RelativeItem().Column(col =>
            {
                ...
                col.Item().Text(string.IsNullOrWhiteSpace(_agreement.Remarks) ? "-" : _agreement.Remarks!);
            });

            row.ConstantItem(220).Column(col =>
            {
                SummaryLine(col, "Subtotal", _agreement.Items.Sum(l => l.Total));
                SummaryLine(col, "Discount ($)", _agreement.Discount);
                SummaryLine(col, "Tax (%)", _agreement.Tax);
                SummaryLine(col, "Tax Amount", _agreement.TaxAmount);
                ...
                col.Item().PaddingTop(4).Row(r =>
                {
                    ...
                    r.RelativeItem().AlignRight().Text($"${_agreement.Total:N2}").Bold().FontSize(12);
                });
            });
        });
    }
