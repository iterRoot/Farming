...
public class PurchaseOrderPdfDocument : IDocument
{
    ...
    public PurchaseOrderPdfDocument(PurchaseOrder order, CompanyEntity? company)
    {
        _order   = order;
        _company = company;
    }
    ...
    private void ComposeHeader(IContainer container)
    {
        container.Row(row =>
        {
            ...
            {
                col.Item().AlignRight().Text("PURCHASE ORDER")
                    .FontSize(18).Bold().FontColor(Colors.Blue.Darken2);

                col.Item().AlignRight().Text($"Doc No: {_order.DocNum}")
                    .FontSize(10).Bold();

                col.Item().AlignRight().Text(_order.Status == "C" ? "Closed" : "Open")
    ...
    private void ComposeVendorInfo(IContainer container)
    {
        container.Row(row =>
        {
            row.RelativeItem().Column(col =>
            {
                ...
                col.Item().Text(_order.Vendor.CardName).Bold();
                col.Item().Text($"Vendor Code: {_order.Vendor.Code}").FontSize(9);
            });
            ...
            row.RelativeItem().Column(col =>
            {
                ...
                col.Item().Row(r =>
                {
                    ...
                    r.RelativeItem().AlignRight().Text(_order.DueDate?.ToString("dd/MM/yyyy") ?? "-");
                });
            });
        });
    }
    ...
    private void ComposeFooterBlock(IContainer container)
    {
        container.Row(row =>
        {
            ...
            {
                SummaryLine(col, "Subtotal", _order.Lines.Sum(l => l.Total));
