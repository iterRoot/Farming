...
public class PurchaseQuotationPdfDocument : IDocument
{
    private readonly PurchaseQuotation _quotation;
    private readonly CompanyEntity?    _company;

    public PurchaseQuotationPdfDocument(PurchaseQuotation quotation, CompanyEntity? company)
    {
    ...
    private void ComposeHeader(IContainer container)
    {
        container.Row(row =>
        {
            ...
            {
                col.Item().AlignRight().Text("PURCHASE QUOTATION")
                    .FontSize(17).Bold().FontColor(Colors.Blue.Darken2);

                col.Item().AlignRight().Text($"Doc No: {_quotation.DocNum}")
                    .FontSize(10).Bold();

                col.Item().AlignRight().Text(_quotation.Status == "C" ? "Closed" : "Open")
    ...
    private void ComposeVendorInfo(IContainer container)
    {
        container.Row(row =>
        {
            row.RelativeItem().Column(col =>
            {
                ...
                col.Item().Text(_quotation.Vendor.CardName).Bold();
                col.Item().Text($"Vendor Code: {_quotation.Vendor.Code}").FontSize(9);
            });
            ...
            row.RelativeItem().Column(col =>
            {
                ...
                col.Item().Row(r =>
                {
                    ...
                    r.RelativeItem().AlignRight().Text(_quotation.DueDate?.ToString("dd/MM/yyyy") ?? "-");
                });
            });
        });
    }
