...
public class ARCreditNotePdfDocument : IDocument
{
    private readonly ARCreditNote   _creditNote;
    private readonly CompanyEntity? _company;

    public ARCreditNotePdfDocument(ARCreditNote creditNote, CompanyEntity? company)
    {
    ...
    private void ComposeHeader(IContainer container)
    {
        container.Row(row =>
        {
            ...
            {
                col.Item().AlignRight().Text("AR CREDIT NOTE")
    ...
    private void ComposeCustomerInfo(IContainer container)
    {
        container.Row(row =>
        {
            row.RelativeItem().Column(col =>
            {
                ...
                col.Item().Text(_creditNote.Customer.CardName).Bold();
                col.Item().Text($"Customer Code: {_creditNote.Customer.Code}").FontSize(9);
            });
            ...
            row.RelativeItem().Column(col =>
            {
                col.Item().Row(r =>
                {
                    ...
                    r.RelativeItem().AlignRight().Text(_creditNote.Type);
                });
                col.Item().Row(r =>
                {
