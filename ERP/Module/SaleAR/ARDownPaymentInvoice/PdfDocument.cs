...
public class ARDownPaymentInvoicePdfDocument : IDocument
{
    private readonly ARDownPaymentInvoice _invoice;
    private readonly CompanyEntity?       _company;

    public ARDownPaymentInvoicePdfDocument(ARDownPaymentInvoice invoice, CompanyEntity? company)
    {
        _invoice = invoice;
        _company = company;
    }
    ...
    private void ComposeHeader(IContainer container)
    {
        container.Row(row =>
        {
            ...
            {
                col.Item().AlignRight().Text("AR DOWN PAYMENT INVOICE")
    ...
    private void ComposeCustomerInfo(IContainer container)
    {
        container.Row(row =>
        {
            row.RelativeItem().Column(col =>
            {
                ...
                col.Item().Text(_invoice.Customer.CardName).Bold();
                col.Item().Text($"Customer Code: {_invoice.Customer.Code}").FontSize(9);
            });
            ...
            row.RelativeItem().Column(col =>
            {
                col.Item().Row(r =>
                {
                    ...
                    r.RelativeItem().AlignRight().Text(_invoice.Type);
                });
                col.Item().Row(r =>
                {
    ...
    private void ComposeLineItemsTable(IContainer container)
    {
        container.Table(table =>
        {
            ...
            var i = 0;
            foreach (var line in _invoice.Items)
            {
                i++;
                var idx = i;
    ...
    private void ComposeFooterBlock(IContainer container)
    {
        container.Row(row =>
        {
            ...
            {
                SummaryLine(col, "Subtotal", _invoice.Items.Sum(l => l.Total));
                SummaryLine(col, "Discount ($)", _invoice.Discount);
                SummaryLine(col, "Tax (%)", _invoice.Tax);
                SummaryLine(col, "Tax Amount", _invoice.TaxAmount);
