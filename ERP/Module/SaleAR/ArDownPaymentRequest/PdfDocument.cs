...
public class ArDownPaymentRequestPdfDocument : IDocument
{
    private readonly ArDownPaymentRequest _request;
    private readonly CompanyEntity?       _company;

    public ArDownPaymentRequestPdfDocument(ArDownPaymentRequest request, CompanyEntity? company)
    {
        _request = request;
        _company = company;
    }
    ...
    private void ComposeHeader(IContainer container)
    {
        container.Row(row =>
        {
            ...
            {
                col.Item().AlignRight().Text("AR DOWN PAYMENT REQUEST")
    ...
    private void ComposeCustomerInfo(IContainer container)
    {
        container.Row(row =>
        {
            row.RelativeItem().Column(col =>
            {
                col.Item().Text("BILL TO").FontSize(9).Bold().FontColor(Colors.Grey.Darken1);
                col.Item().Text(_request.Customer.CardName).Bold();
                col.Item().Text($"Customer Code: {_request.Customer.Code}").FontSize(9);
            });
            ...
            row.RelativeItem().Column(col =>
            {
                col.Item().Row(r =>
                {
                    r.RelativeItem().Text("Type").FontSize(9).FontColor(Colors.Grey.Darken1);
                    r.RelativeItem().AlignRight().Text(_request.Type);
                });
                col.Item().Row(r =>
                {
    ...
    private void ComposeLineItemsTable(IContainer container)
    {
        container.Table(table =>
        {
            ...
            var i = 0;
            foreach (var line in _request.Items)
            {
                i++;
                var idx = i;
    ...
    private void ComposeFooterBlock(IContainer container)
    {
        container.Row(row =>
        {
            ...
            {
                SummaryLine(col, "Subtotal", _request.Items.Sum(l => l.Total));
                SummaryLine(col, "Discount ($)", _request.Discount);
                SummaryLine(col, "Tax (%)", _request.Tax);
                SummaryLine(col, "Tax Amount", _request.TaxAmount);
                SummaryLine(col, "Down Payment (%)", _request.DownPaymentPct);
                SummaryLine(col, "Down Payment Amt", _request.DownPaymentAmt);
