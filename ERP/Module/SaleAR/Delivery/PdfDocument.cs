...
public class DeliveryPdfDocument : IDocument
{
    private readonly Delivery       _delivery;
    private readonly CompanyEntity? _company;

    public DeliveryPdfDocument(Delivery delivery, CompanyEntity? company)
    {
        _delivery = delivery;
        _company  = company;
    }
    ...
    private void ComposeHeader(IContainer container)
    {
        container.Row(row =>
        {
            ...
            row.ConstantItem(180).Column(col =>
            {
                ...
                col.Item().AlignRight().Text($"Doc No: {_delivery.DocNum}")
                    .FontSize(10).Bold();

                col.Item().AlignRight().Text(StatusLabel(_delivery.Status))
    ...
    private void ComposeCustomerInfo(IContainer container)
    {
        container.Row(row =>
        {
            row.RelativeItem().Column(col =>
            {
                col.Item().Text("SHIP TO / CUSTOMER").FontSize(9).Bold().FontColor(Colors.Grey.Darken1);
                col.Item().Text(_delivery.Customer.CardName).Bold();
                col.Item().Text($"Customer Code: {_delivery.Customer.Code}").FontSize(9);
            });
            ...
            row.RelativeItem().Column(col =>
            {
                col.Item().Row(r =>
                {
                    ...
                    r.RelativeItem().AlignRight().Text(_delivery.PostingDate?.ToString("dd/MM/yyyy") ?? "-");
                });
                col.Item().Row(r =>
                {
                    r.RelativeItem().Text("Delivery Date").FontSize(9).FontColor(Colors.Grey.Darken1);
                    r.RelativeItem().AlignRight().Text(_delivery.DeliveryDate?.ToString("dd/MM/yyyy") ?? "-");
                });
            });
        });
    }
    ...
    private void ComposeLineItemsTable(IContainer container)
    {
        container.Table(table =>
        {
            ...
            var i = 0;
            foreach (var line in _delivery.Items)
            {
                i++;
                var idx = i;
    ...
    private void ComposeFooterBlock(IContainer container)
    {
        container.Row(row =>
        {
            row.RelativeItem().Column(col =>
            {
                ...
                col.Item().Text(string.IsNullOrWhiteSpace(_delivery.Remarks) ? "-" : _delivery.Remarks!);
            });

            row.ConstantItem(220).Column(col =>
            {
                SummaryLine(col, "Subtotal", _delivery.Items.Sum(l => l.Total));
                SummaryLine(col, "Discount ($)", _delivery.Discount);
                SummaryLine(col, "Tax (%)", _delivery.Tax);
                SummaryLine(col, "Tax Amount", _delivery.TaxAmount);
                ...
                col.Item().PaddingTop(4).Row(r =>
                {
                    ...
                    r.RelativeItem().AlignRight().Text($"${_delivery.Total:N2}").Bold().FontSize(12);
                });
            });
        });
    }
