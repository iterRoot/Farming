...
public class SaleBlanketAgreementPdfDocument : IDocument
{
    private readonly SaleBlanketAgreement _agreement;
    private readonly CompanyEntity?       _company;

    public SaleBlanketAgreementPdfDocument(SaleBlanketAgreement agreement, CompanyEntity? company)
    {
    ...
    private void ComposeHeader(IContainer container)
    {
        container.Row(row =>
        {
            ...
            {
                col.Item().AlignRight().Text("SALES BLANKET AGREEMENT")
    ...
    private void ComposeCustomerInfo(IContainer container)
    {
        container.Row(row =>
        {
            row.RelativeItem().Column(col =>
            {
                ...
                col.Item().Text(_agreement.Customer.CardName).Bold();
                col.Item().Text($"Customer Code: {_agreement.Customer.Code}").FontSize(9);
            });
