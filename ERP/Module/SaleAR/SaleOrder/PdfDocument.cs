...
public class SaleOrderPdfDocument : IDocument
{
    private readonly SaleOrder      _order;
    private readonly CompanyEntity? _company;

    public SaleOrderPdfDocument(SaleOrder order, CompanyEntity? company)
    {
        _order   = order;
        _company = company;
    }
    ...
    private void ComposeHeader(IContainer container)
    {
        container.Row(row =>
        {
            ...
            {
                col.Item().AlignRight().Text("SALE ORDER")
                    .FontSize(18).Bold().FontColor(Colors.Blue.Darken2);

                col.Item().AlignRight().Text($"Doc No: {_order.DocNum}")
                    .FontSize(10).Bold();

                col.Item().AlignRight().Text(StatusLabel(_order.Status))
    ...
    private void ComposeCustomerInfo(IContainer container)
    {
        container.Row(row =>
        {
            row.RelativeItem().Column(col =>
            {
                ...
                col.Item().Text(_order.Customer.CardName).Bold();
                col.Item().Text($"Customer Code: {_order.Customer.Code}").FontSize(9);
            });
            ...
            row.RelativeItem().Column(col =>
            {
                col.Item().Row(r =>
                {
                    ...
                    r.RelativeItem().AlignRight().Text(_order.PostingDate?.ToString("dd/MM/yyyy") ?? "-");
                });
                col.Item().Row(r =>
                {
                    r.RelativeItem().Text("Delivery Date").FontSize(9).FontColor(Colors.Grey.Darken1);
                    r.RelativeItem().AlignRight().Text(_order.DeliveryDate?.ToString("dd/MM/yyyy") ?? "-");
                });
            });
        });
    }
    ...
    private void ComposeFooterBlock(IContainer container)
    {
        container.Row(row =>
        {
            row.RelativeItem().Column(col =>
            {
                ...
                col.Item().Text(string.IsNullOrWhiteSpace(_order.Remarks) ? "-" : _order.Remarks!);
            });

            row.ConstantItem(220).Column(col =>
            {
                SummaryLine(col, "Subtotal", _order.Items.Sum(l => l.Total));
                SummaryLine(col, "Discount ($)", _order.Discount);
                SummaryLine(col, "Tax (%)", _order.Tax);
                SummaryLine(col, "Tax Amount", _order.TaxAmount);
                ...
                col.Item().PaddingTop(4).Row(r =>
                {
                    ...
                    r.RelativeItem().AlignRight().Text($"${_order.Total:N2}").Bold().FontSize(12);
                });
            });
        });
    }
