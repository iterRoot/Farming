...
public class HouseController : MyController

{
	...
	public IActionResult Gets()
	{
		...
		var results = _mapper.ProjectTo<HouseListResponse>(iQueryable).ToList();
	...
	public IActionResult Get(int id)
	{
		...
		var result = _mapper.Map<HouseListResponse>(item);
		return Ok(result);
	}
	...
	[HttpPost]
	public IActionResult CreatHouses([FromForm] HouseListRequest request)
	{
		if (!ModelState.IsValid)
			return BadRequest(ModelState);
		var sheep = _mapper.Map<House>(request);
		sheep.CreatedAt = DateTime.UtcNow;
		sheep.InActive = false;
		_repository.Add(sheep);
		_repository.Commit();
		var response = _mapper.Map<HouseListResponse>(sheep);
		return CreatedAtAction(nameof(Gets), new { id = response.Id}, response);
	}

	// [HttpPost]
	// public IActionResult CreatHouses()
	// {

	// 	var existed = repository.Existed(e=> e.Id == request.Id);
	// 	if(existed) return Existed(request.id);
	// 	var item = mapper.Map<Houses>(request);
	...
	[HttpPut("{id:int}")]
	public async Task<IActionResult> Update(int id, [FromForm] HouseUpdateRequest request)
	{
		var item = _repository.GetSingle(e => e.Id == id);
		if (item == null)
		{
